...
import newjohn.com.pigpen.bean.DeviceDb;
import newjohn.com.pigpen.bean.DeviceDbDao;
import newjohn.com.pigpen.bean.HistoryData;
import newjohn.com.pigpen.bean.PigpenHisData;
import newjohn.com.pigpen.bean.PigpenHisDataToDB;
import newjohn.com.pigpen.bean.PigpenHisDataToDBDao;
import newjohn.com.pigpen.bean.Sensor;
import newjohn.com.pigpen.bean.SensorDao;
import newjohn.com.pigpen.excel.ExcelUtils;
import newjohn.com.pigpen.global.Global;
...
import newjohn.com.pigpen.ui.CustomDatePicker;
import okhttp3.Call;
...
public class HistoryActivity extends BaseActivity implements  AutoListView.OnRefreshListener, AutoListView.OnLoadListener, AdapterView.OnItemClickListener {
   ...
   // List<HistoryData> historyDatas=new ArrayList<HistoryData>();//Gson解析使用
    List<PigpenHisData> pigpenHisDatas=new ArrayList<>();
    //SensorDao sensorDao;
    PigpenHisDataToDBDao pigpenHisDataToDBDao;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pigpenHisDataToDBDao=MyApplication.getMyApplication().getDaoSession().getPigpenHisDataToDBDao();
    ...
    private void initView() {
        ...
        adapter = new ListViewScrollAdapter(this, list, R.layout.auto_listview_item,
                new String[] { "area", "deviceNum", "value", "dateTime"},
                new int[] { R.id.item_title, R.id.item_data1, R.id.item_data2, R.id.item_data3},
...
//                Toast.makeText(HistoryActivity.this, "你点击的是:"+dl.get(postion), Toast.LENGTH_SHORT).show();
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.excel) {
            ...
            List<PigpenHisDataToDB> pigpenHisDataToDBs =pigpenHisDataToDBDao.loadAll();

            try {
                ExcelUtils.writeExcel(pigpenHisDataToDBs);
    ...
    public String sensorType(String deviceNum){
        if (deviceNum.indexOf("CO")!=-1){
            return "ppm";
        }
        else if (deviceNum.indexOf("光照")!=-1){
            return "lux";
        }
        else if (deviceNum.indexOf("粉尘")!=-1){
            return "ug/m3";
        }
        else if (deviceNum.indexOf("温度")!=-1){
            return "℃";
        }
        else if (deviceNum.indexOf("湿度")!=-1){
            return "%RH";
        }
        else {
            return "单位未知";
